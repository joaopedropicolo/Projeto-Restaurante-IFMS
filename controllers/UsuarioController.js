...
class UsuarioController {
  ...
  static async login(req, res) {
    ...
    const token = jwt.sign({ id: usuario.id }, process.env.SENHA_TOKEN, { expiresIn: "2h" });

    res.json({ msg: "Autenticado com sucesso!", token });
  }
  ...
  static async verificaIsAdmin(req, res, next) {
    ...
    if (!usuario || usuario.tipo !== "admin") {
      return res.status(403).json({ msg: "Acesso negado! Você não é administrador." });
    }
